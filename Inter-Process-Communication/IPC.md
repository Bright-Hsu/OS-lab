...
**问题：先猜想一下这个程序的运行结果。分析管道通信是怎样实现同步与互斥的，然后按照注释里的要求把代码补充完整，运行程序。** **修改程序并运行，体会互斥锁的作用，比较有锁和无锁程序的运行结果，并解释之。**

把代码补全，料想到输出必然是200个连续的1和200个连续的2，但是1和2的先后顺序不能确定，可以看出程序输出如下：

![image-20220319232050840](https://gitee.com/bright_xu/blog-image/raw/master/img/image-20220319232050840.png)

把互斥锁删除掉，可以看到1和2不是连续的，是混乱的，随机的：

![电子设备的屏幕  中度可信度描述已自动生成](https://gitee.com/bright_xu/blog-image/raw/master/img/clip_image004.png)

可以看出，为了协调双方的通信，实现同步与互斥，管道首先要确定对方是否存在，只有确定了对方已存在时，才能进行通信。然后管道通信的同步与互斥按照以下机制执行：
